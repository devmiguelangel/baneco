...
header("Cache-Control: no-cache, must-revalidate, post-check=0, pre-check=0", false);
...
?>
    ...
    --><ul id="user-menu">
      <li>
        <a href="#">
          <span class="login-icon"></span><span class="login-txt">
            <?=$user;?>
            <br>
            <span>
              <?=ucwords(mb_strtolower($user_name, 'utf-8'));?>
            </span>
          </span>
        </a>
        <ul>
...
?>
    ...
    <?php if ($tokenM === true): ?>
      ...
      <?php if ($user_type === 'FAC' || $user_type === 'ROOT'): ?>
        <?php if ($rowMenu['producto'] !== 'TRD' && $rowMenu['producto'] !== 'TH'): ?>
          <li><a href="index.php?ms=<?=md5('MS_'.$rowMenu['producto']);?>&page=<?=md5('P_fac');?>" class="link-pr">
            Registros Facultativos
          </a></li>
        <?php endif ?>
      <?php elseif ($user_type === 'IMP' && $link->verify_implant($_SESSION['idEF'], $rowMenu['producto']) === true): ?>
