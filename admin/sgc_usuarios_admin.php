...
<?php	
	...
	if(isset($_POST['accionUsuarios'])) {
		...
		if($errFlag) {
			...
			$insert = "INSERT INTO s_usuario(id_usuario, usuario, password, nombre, email, id_tipo, id_depto, cargo, activado, id_agencia, fono_agencia, fecha_creacion) "
			."VALUES('".$id_unico."', '".$usuario."', '".$encrip_pass."', '".$nombre."', '".$email."', ".$id_tipo.", ".$depto_regional.", '".$cargo."', 1, ".$id_agencia.", '".$fono_agencia."', curdate())";
			//echo $insert;
       ...
       $(document).ready(function(e) {
		   ...
		   $('#frmUsuario').submit(function(e){
			  ...
			  $(this).find('.requerid').each(function(){
				   ...
				   if(cargo!=''){
					   if(cargo.match(/^[a-zA-Z·ÈÌÛ˙‡ËÏÚ˘¿»Ã“Ÿ¡…Õ”⁄Ò—¸‹\s\D]+$/)){
						    $('#errorcargo').slideUp('slow');
					   }else{
						    sum++;
							$('#errorcargo').slideDown('slow');
							$('#errorcargo').html('ingrese solo caracteres');
					   }
				   }
		...
		<div class="da-panel-content">
			<form class="da-form" name="frmUsuario" action="" method="post" id="frmUsuario">
				...
				<div class="da-form-row">
					...
					<div class="da-form-item large">
						<input class="textbox requerid" type="text" name="txtCargo" id="txtCargo" style="width: 250px;"  value="" autocomplete="off"/>
		...
		if(isset($_POST['accionEditar'])) {
			...
			if($errFlag) {
				...
				$update = "UPDATE s_usuario as su 
						   ...
						   SET su.nombre='".$nombre."', cargo='".$cargo."',";
       ...
       $(document).ready(function(e) {
		   ...
		   $('#frmUsuario').submit(function(e){
			  ...
			  $(this).find('.requerid').each(function(){
				   ...
				   if(cargo!=''){
					   if(cargo.match(/^[a-zA-Z·ÈÌÛ˙‡ËÏÚ˘¿»Ã“Ÿ¡…Õ”⁄Ò—¸‹\s\D]+$/)){
						    $('#errorcargo').hide('slow');
					   }else{
						    sum++;
							$('#errorcargo').show('slow');
							$('#errorcargo').html('ingrese solo caracteres');
					   }
				   }
		  ...
		  echo'<div class="da-form-row">
		   ...
		   echo'<div class="da-form-row">
				...
				<div class="da-form-row">
					<label style="text-align:right;"><b><span lang="es">Cargo</span></b></label>
					<div class="da-form-item large">
						<input class="textbox requerid" type="text" name="txtCargo" id="txtCargo" style="width: 250px;"  value="'.$fila['cargo'].'" autocomplete="off"/>
						<span class="errorMessage" id="errorcargo" lang="es"></span>
					</div>
				</div>';
