<?php
	 ...
	 if($num_regi==0){
	  ...
	  $sql_orden="SELECT
				 ...
				 FROM
					s_pregunta
				 WHERE
					id_ef_cia='".$_POST['id_ef_cia']."' and producto='".base64_decode($_POST['txtProducto'])."'	   
				 ORDER BY
					orden ASC";
	  ...
	  $insert ="INSERT INTO s_pregunta(id_pregunta, pregunta, orden, respuesta, producto, id_ef_cia) "
			  ."VALUES(NULL, '".$txtpregunta."', ".$orden.", ".$_POST['txtRespuesta'].", '".base64_decode($_POST['txtProducto'])."', '".$_POST['id_ef_cia']."')";
	  ...
	  $pregunta = $conexion->real_escape_string($_POST['txtPregunta']);
	  $respuesta = $conexion->real_escape_string($_POST['txtRespuesta']);
	  $update = "update s_pregunta set pregunta='".$pregunta."', respuesta=".$respuesta." where id_pregunta=".$_POST['id_pregunta']." and id_ef_cia='".$_POST['id_ef_cia']."' and producto='".$_POST['txtProducto']."';"; 
	  ...
	  if($_POST['text']=='activar'){
		  $update = "update s_pregunta set activado=1 where id_pregunta=".$_POST['id_pregunta']." and id_ef_cia='".$_POST['id_ef_cia']."' and producto='".base64_decode($_POST['producto'])."';"; 
		  
		   
	  }elseif($_POST['text']=='desactivar'){
		  $update = "update s_pregunta set activado=0 where id_pregunta=".$_POST['id_pregunta']." and id_ef_cia='".$_POST['id_ef_cia']."' and producto='".base64_decode($_POST['producto'])."';"; 
  ...
  }elseif($_POST['opcion']=='crea_plan'){
	   $id_plan = date('U');
	   $nombre = $conexion->escape_string($_POST['txtNombre']);
	   $descripcion = $conexion->escape_string($_POST['txtDescripcion']);
	   $prima_mes = $conexion->escape_string($_POST['txtPrimaMes']);
	   $prima_anual = $conexion->escape_string($_POST['txtPrimaAnual']);
	   $plan = array(0=>array('cov'=>$_POST['txtOp1'], 'rank'=>$_POST['txtBs1']), 1=>array('cov'=>$_POST['txtOp2'], 'rank'=>$_POST['txtBs2']), 2=>array('cov'=>$_POST['txtOp3'], 'rank'=>$_POST['txtBs3']));
	   
	   $insert ="INSERT INTO s_plan(id_plan, id_ef, nombre, descripcion, prima_mensual, prima_anual, plan, producto) "
			  ."VALUES(".$id_plan.", '".base64_decode($_POST['id_ef'])."', '".$nombre."', '".$descripcion."', ".$prima_mes.", ".$prima_anual.", '".$conexion->real_escape_string(json_encode($plan))."', '".base64_decode($_POST['txtProducto'])."')";
	   if($conexion->query($insert)===TRUE){
		  echo 1; 
	   }else{
		  echo 2;
	   }  
  }elseif($_POST['opcion']=='edita_plan'){
	  $nombre = $conexion->real_escape_string($_POST['txtNombre']);
	  $descripcion = $conexion->real_escape_string($_POST['txtDescripcion']);
	  $prima_mes = $conexion->real_escape_string($_POST['txtPrimaMes']);
	  $prima_anual = $conexion->real_escape_string($_POST['txtPrimaAnual']);
	   	  
	  $plan = array(0=>array('cov'=>$_POST['txtOp1'], 'rank'=>$_POST['txtBs1']), 1=>array('cov'=>$_POST['txtOp2'], 'rank'=>$_POST['txtBs2']), 2=>array('cov'=>$_POST['txtOp3'], 'rank'=>$_POST['txtBs3']));
	  
	  $update = "UPDATE s_plan SET nombre = '".$nombre."', descripcion = '".$descripcion."', prima_mensual = ".$prima_mes.", prima_anual=".$prima_anual.", producto='".base64_decode($_POST['txtProducto'])."', plan='".$conexion->real_escape_string(json_encode($plan))."' WHERE id_plan = ".$_POST['id_plan']." and id_ef = '".base64_decode($_POST['id_ef'])."' and producto = '".base64_decode($_POST['txtProducto'])."';";
	  //echo $update; 
	  if($conexion->query($update) === TRUE){
		echo 1; 
	  }else{
