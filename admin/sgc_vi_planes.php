                	...
                	<div id="da-content-area">
                    	...
                    	<div class="grid_4">
                           <?php
								...
								mostrar_lista_planes($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion);
...
function mostrar_lista_planes($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion){
?>
   ...
   $(function(){
	   $("a[href].eliminar").click(function(e){
		   ...
		   jConfirm("¿Esta seguro de eliminar el registro?", "Eliminar registro", function(r) {
				//alert(r);
				if(r) {
						var dataString ='id_plan='+id_plan+'&id_ef='+id_ef+'&producto='+producto+'&opcion=elimina_plan';
						$.ajax({
							   async: true,
							   cache: false,
							   type: "POST",
...
if($tipo_sesion=='ROOT'){
   ...
   if($resef = $conexion->query($selectEf,MYSQLI_STORE_RESULT)){
		...
		if($num_regi_ef>0){
			  $rg_data = $resef->fetch_array(MYSQLI_ASSOC);
					  ...
					  <div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
						  <ul class="action_user">
							  <li style="margin-right:6px;">
								 <a href="adicionar_registro.php?opcion=crear_plan&id_ef='.base64_encode($rg_data['id_ef']).'&producto='.$_GET['producto'].'" class="da-tooltip-s various fancybox.ajax" title="<span lang=\'es\'>Añadir nuevo plan</span>">
								 <img src="images/add_new.png" width="32" height="32"></a>
							  </li>
						  </ul>
					  </div>
				   </div>';
				  if ($resef->data_seek(0) === TRUE) {  
					  while($regief = $resef->fetch_array(MYSQLI_ASSOC)){	
							  $selectFor="select
										...
										from
										  s_plan as sp
										  inner join s_sgc_home as ssh on (ssh.id_ef=sp.id_ef and ssh.producto=sp.producto)
										where
										   sp.id_ef='".$regief['id_ef']."' and ssh.producto='".base64_decode($_GET['producto'])."';";
							  $res = $conexion->query($selectFor,MYSQLI_STORE_RESULT);	
							  echo'
							  <div class="da-panel collapsible" style="width:100%;">
								  <div class="da-panel-header">
									  <span class="da-panel-title">
										  <img src="images/icons/black/16/list.png" alt="" />
										  <b>'.$regief['nombre'].'</b> - <span lang="es">Listado de Registros Planes</span>
									  </span>
								  </div>
								  <div class="da-panel-content">
									  <table class="da-table">
										  <thead>
											  <tr>
												  <th style="width:150px;"><b><span lang="es">Nombre</span></b></th>
												  <th style="width:350px;"><b><span lang="es">Descripcion</span></b></th>
												  <th style="width:100px;"><b><span lang="es">Prima Mensual (Bs.)</span></b></th>
												  <th style="width:100px;"><b><span lang="es">Prima Anual (Bs.)</span></b></th>
												  <th style="width:350px;"><b><span lang="es">Plan</span></b></th>
										  ...
										  <tbody>';
											...
											if($num>0){
												  while($regi = $res->fetch_array(MYSQLI_ASSOC)){
													  $plan = json_decode($regi['plan'], true);
													  echo'<tr>
															  <td>'.$regi['nombre'].'</td>
															  <td>'.$regi['descripcion'].'</td>
															  <td>'.$regi['prima_mensual'].'</td>
															  <td>'.$regi['prima_anual'].'</td>
															  <td>';
														       echo $plan[0]['cov'].' Hasta Bs. '.$plan[0]['rank'];
															   echo '<br/>';
															   echo $plan[1]['cov'].' Hasta Bs. '.$plan[1]['rank'];
															   echo '<br>';
															   echo $plan[2]['cov'].' Hasta Bs. '.$plan[2]['rank']; 
														 echo'</td>
															  <td>'.$regi['producto_nombre'].'</td>
															  <td class="da-icon-column">
																 <ul class="action_user">
																	<li style="margin-right:5px;"><a href="adicionar_registro.php?opcion=editar_plan&id_plan='.base64_encode($regi['id_plan']).'&id_ef='.base64_encode($regief['id_ef']).'&producto='.$_GET['producto'].'" class="edit da-tooltip-s various fancybox.ajax" title="<span lang=\'es\'>Editar</span>"></a></li>
																	<li><a href="#" class="eliminar da-tooltip-s" title="<span lang=\'es\'>Eliminar</span>" id="'.$regi['id_plan'].'|'.$regief['id_ef'].'|'.base64_decode($_GET['producto']).'"></a></li>';
															echo'</ul>	
															  </td>
														  </tr>';
												  }
