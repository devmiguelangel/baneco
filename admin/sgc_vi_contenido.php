...
<script type="text/javascript">
  ...
  $(function(){
    ...
    <?php if($msg!=''){ ?>
		 ...
		 setTimeout( "$(location).attr('href', 'index.php?l=<?=$l;?>&var=<?=$var;?>&producto=<?=$producto;?>');",5000 );
	<?php }?>
			...
			while($regief = $resef->fetch_array(MYSQLI_ASSOC)){
				$selectCont="select
							...
							from
							  s_sgc_home
							where
							  id_ef='".$regief['id_ef']."' and producto='".base64_decode($_GET['producto'])."';";
				...
				<div class="da-panel collapsible">
					<div class="da-panel-header">
						<span class="da-panel-title">
							...
							<b>'.$regief['nombre'].'</b> - <span lang="es">Contenido '.$row['producto_nombre'].'</span>
						</span>
					</div>
										...
										echo'<tr>
										   ...
										   echo'</td>
												...
												<td class="da-icon-column">
												   <ul class="action_user">
													  <li style="margin-right:5px;"><a href="?l='.$_GET['l'].'&editar=v&var='.$_GET['var'].'&idhome='.base64_encode($regi['id_home']).'&id_ef='.base64_encode($regief['id_ef']).'&producto='.$_GET['producto'].'" class="edit da-tooltip-s" title="<span lang=\'es\'>Editar</span>"></a></li>';
											  echo'</ul>	
												</td>
											</tr>';
									}
	...
	if(isset($_POST['accionGuardar'])) {
		...
		if($errFlag) {
			...
			mostrar_editar_contenidohome($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $id_ef_sesion, $conexion, $errArr);
			   ...
			   $update.="html='".$contenido."' WHERE id_home = '".$idhome."' and id_ef ='".$id_ef."' and producto='".base64_decode($_GET['producto'])."' LIMIT 1;";
			...
			if($conexion->query($update)===TRUE){
			   ...
			   header('Location: index.php?l='.$_GET['l'].'&var='.$_GET['var'].'&op=1&msg='.$mensaje.'&producto='.$_GET['producto']);
			   exit;
			} else{
			   $mensaje="Hubo un error al ingresar los datos, consulte con su administrador ".$conexion->errno.": ". $conexion->error;
			   header('Location: index.php?l='.$_GET['l'].'&var='.$_GET['var'].'&op=2&msg='.$mensaje.'&producto='.$_GET['producto']);
			   exit;
			}  
...
<?php
  ...
  $selectImg="select
				...
				where
				  sh.id_home='".$idhome."' and sh.id_ef='".$id_ef."' and sh.producto='".base64_decode($_GET['producto'])."' and sef.activado=1;";
  //echo $selectImg;
 ...
 echo'
	<div class="da-panel collapsible">
		<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
			<ul class="action_user">
				<li style="margin-right:6px;">
				   <a href="?l='.$_GET['l'].'&var='.$_GET['var'].'&producto='.$_GET['producto'].'" class="da-tooltip-s" title="<span lang=\'es\'>Volver</span>">
