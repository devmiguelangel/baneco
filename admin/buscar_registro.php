<?php
  ...
  }elseif($_POST['opcion']=='buscar_producto_AdCia'){
	 //VISUALIZA LOS PRODUCTOS DE ACUERDO A LA ENTIDAD FINANCIERA
     //PARA AGREGAR LA COMPAÑIA DE SEGUROS A UN DETERMINADO PRODUCTO
	  
	    $select="select 
					sh.id_home, sh.id_ef, sh.producto, sh.producto_nombre
				from
					s_sgc_home as sh
				where
					id_ef = '".$_POST['idefin']."'
						...
						and not exists( select 
							sec.producto
						from
							s_ef_compania as sec
						where
							sec.id_ef = sh.id_ef
								and sec.producto = sh.producto);";
		$sql = $conexion->query($select,MYSQLI_STORE_RESULT);
		if($sql->num_rows>0){
			echo'<select name="producto" id="producto" class="required requerid" style="width:170px;">';
						echo'<option value="">Seleccionar...</option>';
