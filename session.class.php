...
class Session{
    ...
    function Session(){
        ...
        if (strpos($domain, '.app') !== false) {
          # code...
        } elseif (strpos($domain, 'baneco') === false) {
            $path = '/baneco';
        }
    ...
    {
        if (!isset($_COOKIE['BanecoUser']) 
            && empty($_SESSION['idUser']) === false
            && isset($_SESSION['idEF'])) {
            //setcookie('BanecoUser[user]', $_SESSION['idUser'], time() + 1 * 24 * 60 * 60);
            setcookie('BanecoUser[user]', $_SESSION['idUser'], time() + $this->max_time, '/', '', false, true);
            setcookie('BanecoUser[ef]', $_SESSION['idEF'], time() + $this->max_time, '/', '', false, true);
    ...
    public function getSessionCookie()
    {
        if (isset($_COOKIE['BanecoUser']) 
            ...
            $data_idepro = $_COOKIE['BanecoUser'];
            /*$ideproUser['user'] = htmlspecialchars($_COOKIE['BanecoUser[user]']);
            $ideproUser['ef'] = htmlspecialchars($_COOKIE['BanecoUser[ef]']);*/
    ...
    public function remove_session()
    {
        ...
        if (isset($_COOKIE['BanecoUser'])) {
            setcookie('BanecoUser', '', time() - 3600, '/', '', false, true);
            setcookie('BanecoUser[user]', '', time() - 3600, '/', '', false, true);
            setcookie('BanecoUser[ef]', '', time() - 3600, '/', '', false, true);
            /*setcookie('BanecoUser[user]', '', time() - 3600);
            setcookie('BanecoUser[ef]', '', time() - 3600);
            */
        }
    }
