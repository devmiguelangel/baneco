...
$arrBS = array(0 => 0, 1 => 'R', 2 => 'Error: No se pudo procesar la Cotización');
...
if(isset($_POST['ms']) && isset($_POST['page']) && isset($_POST['pr'])){
	...
	if($pr === 'AP|01'){
		...
		if ($cp !== NULL) {
			...
			$record = $link->getRegistrationNumber($_SESSION['idEF'], 'AP', 0);
			
			$sql = 'insert into s_ap_cot_cabecera
			...
			if ($link->query($sql) === true) {
				$arrBS[0] = 1;
				$arrBS[1] = 'ap-quote.php?ms=' . $ms . '&page=' . $page . '&pr=' 
					. base64_encode('AP|02') . '&idc=' . base64_encode($idc);
			...
			if ($ca[0] === true) {
				if ($idc === NULL) {
					$idc = uniqid('@S#1$2013',true);
					$record = $link->getRegistrationNumber($_SESSION['idEF'], 'BS', 0);
					
					$sql = 'insert into s_bs_cot_cabecera 
				...
				if ($link->query($sql) === TRUE) {
					$arrBS[0] = 1;
					$arrBS[1] = 'de-quote.php?ms='.$ms.'&page='.$page.'&pr='.base64_encode('BS|02').'&idc='.base64_encode($idc);
					$arrBS[2] = $lang_quote['mess_03'];
				} else {
					$arrBS[2] = $lang_quote['mess_43'] . '. !';
				}
			} else {
				$arrBS[2] = $lang_quote['mess_44'] . ' '.number_format($ca[1],2,'.',',').' '.$dl_currency;
			}*/
		}
