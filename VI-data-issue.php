...
if (isset($_GET['print'])) {
    if ($_GET['print'] === sha1('p_print')) {
        ...
        if (($rs_cia = $link->getFinancialInstitutionCompany($_SESSION['idEF'], 'VI')) !== false) {
...
$max_item = 0;
if (($rowBS = $link->get_max_amount_optional($_SESSION['idEF'], 'VI')) !== FALSE) {
...
switch($flag){
    case md5('i-new'):
        $action = 'VI-issue-record.php';
        $title = 'Emisión de Póliza de Seguro de Vida Individual';
        $title_btn = 'Guardar';
...
case 1:
    ...
    $sql .= '
        ...
        "" as cargo_periodo,
        "" as voucher,
        if(scl.peso is null, 0, scl.peso) as cl_peso,
        if(scl.estatura is null, 0, scl.estatura) as cl_estatura,
    ...
    from
        s_vi_cot_cabecera as sdc
            inner join
        s_vi_cot_detalle as sdd ON (sdd.id_cotizacion = sdc.id_cotizacion)
            inner join
        s_vi_cot_cliente as scl ON (scl.id_cliente = sdd.id_cliente)
            inner join 
...
if ($sw !== 1){
    ...
    from
        s_vi_em_cabecera as sde
            left join
        s_vi_cot_cabecera as sdet ON (sdet.id_cotizacion = sde.id_cotizacion)
            inner join
...
<a href="certificate-detail.php?idc=<?=base64_encode($idc);?>&cia=<?=
    $_GET['cia'];?>&type=<?=base64_encode('PRINT');?>&pr=<?=base64_encode('VI');?>" 
...
while($row = $rs->fetch_array(MYSQLI_ASSOC)){
    ...
    if ($sw !== 1){
        ...
        $pledge = (boolean)$row['pledge'];

        if ($pledge) {
            $pledge_dis   = 'display: block;';
            $pledge_cb    = 'checked';
...
?>
        ...
        <div class="content-input">
            <input type="text" id="dc-<?=$cont;?>-doc-id" name="dc-<?=$cont;?>-doc-id" autocomplete="off" 
                value="<?=$row['cl_ci'];?>" class="required dni fbin" <?=$read_new . ' ' . $read_edit;?>>
        </div><br>
        
        <label>Complemento: </label>
        <div class="content-input">
            <input type="text" id="dc-<?=$cont;?>-comp" name="dc-<?=$cont;?>-comp" autocomplete="off" 
                value="<?=$row['cl_complemento'];?>" class="not-required dni fbin" style="width:60px;" <?=$read_new;?>>
        </div><br>
...
?>
        ...
        <div class="content-input">
            <input type="text" id="dc-<?=$cont;?>-work" name="dc-<?=$cont;?>-work" autocomplete="off" 
                value="<?=$row['cl_direccion_laboral'];?>" class="not-required fbin" 
                <?=$read_save . ' ' . $read_save_2;?>>
        </div><br>
    </div><!--
    --><div class="form-col">
...
?>
        ...
        <div class="content-pledge" style="<?= $pledge_dis ;?>">
            <label>
                <input class="check" type="checkbox" <?= $pledge_cb ;?>
                    id="dc-pledge" name="dc-pledge" value="1">
                    Pignoración
            </label><br>
            <label>Número de Caso: </label>
            <div class="content-input">
                <input type="text" id="dc-case-number" name="dc-case-number" 
                    autocomplete="off" value="<?= $case_number ;?>" 
                    class="not-required number fbin" readonly>
            </div>
        </div>
...
?>
        ...
        <div class="content-input">
            <select id="dc-plan" name="dc-plan" 
                ...
                <option value="">Seleccione...</option>
<?php
if (($rsOcc = $link->get_plan($_SESSION['idEF'], 'VI')) !== FALSE) {
    while($rowOcc = $rsOcc->fetch_array(MYSQLI_ASSOC)){
        if ($rowOcc['id_plan'] === $row['cl_plan']) {
            echo '<option value="' . $rowOcc['id_plan'] 
                . '" selected>' . $rowOcc['nombre'] . '</option>';
        } else {
            echo '<option value="' . $rowOcc['id_plan'] . '">' 
                . $rowOcc['nombre'] . '</option>';
        }
    }
}
?>
...
if (($rsPl = $link->get_plan($_SESSION['idEF'], 'VI')) !== false) {
...
if ($sw !== 1) {
    ...
    from
        s_vi_beneficiario as sab
            inner join
        s_vi_em_detalle as sdd ON (sdd.id_detalle = sab.id_detalle)
    where
    ...
    <div class="form-col">
        ...
        <div class="content-input">
            <input type="text" id="dsp-<?=$cont;?>-dni-<?=$nbn;?>" 
                ...
                class="not-<?=$beneficiaryData[$nbn]['class'];?> number fbin" <?=$read_save;?>>
        </div><br>
...
?>
    <br>
<?php if ($print === true): ?>
    <h4>Seguro de Vida Individual - Cuestionario</h4>
    <div>
...
<?php
        if (($rsQs = $link->get_question($_SESSION['idEF'], 'VI')) !== FALSE) {
            $arr_qs = array();
            if ($sw !== 1) {
                if (($row_qs = $link->get_question_be($_GET['ide'], 'VI')) !== false) {
    ...
    <div class="form-col">
        ...
        <div class="content-input" style="display: none;">
            <select id="di-policy" name="di-policy" class="required fbin" <?=$read_save;?>>
                <!-- <option value="">Seleccione...</option> -->
<?php
if (($rsPl = $link->get_policy($_SESSION['idEF'], 'VI')) !== false) {
    while($rowPl = $rsPl->fetch_array(MYSQLI_ASSOC)){
...
?>
    ...
    --><div class="form-col">
<?php
if ($sw === 2 && ($cr_payment === 'CO' || $cr_payment === 'DM')) {
?>
...
$(document).ready(function(e) {
    ...
    <?php if ($sw === 1): ?>
    $('#dc-pledge').on('ifChanged', function (e) {
        var caseNumber = $('#dc-case-number');

        if ($(this).is(':checked')) {
            caseNumber.prop('readonly', false);
            caseNumber.removeClass('not-required').addClass('required');
        } else {
            caseNumber.prop('readonly', true).prop('value', '');
            caseNumber.removeClass('required').addClass('not-required');
        }
    });
    <?php else: ?>
    $('#dc-pledge').prop('disabled', true);
    <?php endif ?>
    ...
    $("#dc-edit").click(function(e){
        e.preventDefault();
        location.href = 'vi-quote.php?ms=<?=$_GET['ms'];?>&page=<?=$_GET['page'];?>&pr=<?=$_GET['pr'];?>&ide=<?=base64_encode($ide);?>&flag=<?=md5('i-edit');?>&cia=<?=$_GET['cia'].$target.$print_url;?>';
    });
<?php
switch($sw){
    case 1:
