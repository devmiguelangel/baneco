            ...
            if (empty($ID) === false) {
                ...
                if (($rs = $link->query($sql, MYSQLI_STORE_RESULT)) !== false) {
                    if ($rs->num_rows === 1) {
                        ...
                        $ws_cnx = $link->checkWebService($_SESSION['idEF'], 'VI');
                            ...
                            if ($ws_cnx && ($payment === 'CO' || $payment === 'DM')) {
                                ...
                                if ($link->query($sql) === true) {
                                    if ($target === true) {
                                        ...
                                        $collections = $link->verifyCollections('vi', $ID);

                                        if ($collections === 0) {
                                            $queryset = 'insert into s_vi_cobranza
                                            ...
                                            if ($link->query($queryset) === true && $link->affected_rows > 0) {
                                                ...
                                                $arrBS[1] = 'certificate-policy.php?ms=' . $_POST['ms'] 
                                                    . '&page=' . $_POST['page'] . '&pr=' . base64_encode('VI') 
