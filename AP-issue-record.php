...
if ($token === true){
    ...
    if (isset($_POST['flag']) && (isset($_POST['de-idc']) || isset($_POST['de-ide'])) 
        ...
        if ($_POST['pr'] === base64_encode('AP|05')){
            ...
            $per_mess = $account_mess = $rank_mess = $policy_mess = $pledge_mess = '';
            ...
            if ($sw !== 0){
                ...
                if ($nCl > 0 && $nCl <= $max_item){     // VERIFICAR SI EXISTEN CLIENTES
                    ...
                    $dcr_prima = 0;
                    if (($plan = $link->get_plan($_SESSION['idEF'], 'AP', $dcr_plan)) !== false) {
                    ...
                    if ($dcr_payment === 'DA' && $sw === 1) {
                        if (isset($_POST['dc-pledge'])) {
                            ...
                            if ($ws_cnx) {
                                ...
                                if ($ws->ws_connect($params) > 0) {
                                    
                                } else {
                                    $pledge_mess = 'No se pudo registrar la orden 
                                        de pignoración Nro. ' . $dc_case_number;
                    ...
                    while ($cont < $nCl) {
                        ...
                        $arr_cl[$cont]['cl-hand'] = 
                            $link->real_escape_string(trim($_POST['dc-'.$cont.'-hand']));
                    ...
                    if ($sw === 1){
                        ...
                        $sqlD = 'INSERT INTO s_ap_em_detalle 
                        ...
                        $sqlRs = 'insert into s_ap_em_respuesta 
                        (id_respuesta, id_detalle, respuesta, observacion)
                        values ';
                            
                        $sqlBN = 'INSERT INTO s_ap_beneficiario 
                        ...
                        while($k < $nCl){
                            ...
                            if ($flagCl === false) {
                                ...
                                (id_cliente, id_ef, codigo_be, tipo, razon_social, paterno, 
                                    ...
                                    email, peso, estatura, genero, edad, mano) 
                                VALUES (
                                    "' . $arr_cl[$k]['cl-id'] . '", 
                        ...
                        if ($link->query($sqlC) === true) {
                            ...
                            $arrDE[2] = 'La Póliza no pudo ser registrada'.$sqlC;
                        }
                    } elseif ($sw === 3){
                            ...
                            while($k < $nCl){
                                ...
                                for ($i = 1; $i <= $beneficiary ; $i++) {
                                    if ($arr_cl[$k]['cl-bn-active-' . $i]) {
                                        ...
                                        $sqlBN = 'delete from s_ap_beneficiario
                                        where
                                            id_beneficiario = "' . $arr_cl[$k]['cl-bn-idb-' . $i] . '"
                                                and cobertura = "BN" ;';
