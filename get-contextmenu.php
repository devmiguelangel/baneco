...
if(isset($_GET['ide'])){
	...
	if($quote === FALSE){
		...
		if(($rs = $link->query($sql,MYSQLI_STORE_RESULT))){
			if($rs->num_rows === 1){
				...
				if($token === 1){
					...
					$menu .= '<li><a href="mark-read-unread.php?ide=' 
						. base64_encode($ide) . '&idvh=' . base64_encode($idVh) 
						. '&flag=' . base64_encode((int)$row['leido']) . '&fwd=' 
						. base64_encode($product) . '" class="fancybox fancybox.ajax fde-process">' 
						. $txtMark . '</a></li>';
				}
			}
		}
		//echo $product;
		$link->close();
		if($token !== 3 && $token !== 7 && (boolean)$row['emitir'] === true) {
			$menu .= '<li><a href="certificate-detail.php?ide=' . base64_encode($ide) 
				. '&pr=' . base64_encode($product) . '&type=' . base64_encode('PRINT') 
				. '&category=' . $category . '" class="fancybox fancybox.ajax observation">Ver ' 
				. $titleCert . '</a></li>';
			
			if ($product === 'DE' && $modality === false) {
				/*$menu .= '<li><a href="certificate-detail.php?ide=' . base64_encode($ide) 
					. '&pr=' . base64_encode($product) . '&type=' . base64_encode('PRINT') 
					. '&category=' . $category2 . '" class="fancybox fancybox.ajax observation">Ver ' 
					. $titleCert2 . '</a></li>';*/
			}
		}
		...
		if ($product !== 'DE' && (boolean)$row['pp'] === false) {
			$menu .= '<li><a href="certificate-detail.php?idc=' . base64_encode($idc) 
				...
				. '&type=' . base64_encode('PRINT') . '" class="fancybox fancybox.ajax observation">
				Ver Slip de Cotización</a></li>';
		}
			...
			if(($rs = $link->query($sql, MYSQLI_STORE_RESULT))){
				if ($rs->num_rows === 1) {
					...
					if ($token === 3) {
						if($limit === 0) {
							$pr_href = $product.'|';
							if ($product === 'DE' || $product === 'AP' || $product === 'VI') {
								$pr_href .= '04';
							} else {
								$pr_href .= '03';
							}
							
							$menu .= '<li><a href="' . $pr . '-quote.php?ms=' 
								. md5('MS_' . $product) . '&page=' . md5('P_quote') 
								. '&pr=' . base64_encode($pr_href) . '&idc=' 
								. base64_encode($idc) . '">Emitir Cotización</a></li>';
						} elseif($limit === 1) {
							$menu .= '<li><span class="cm-link">
								<span class="view-ste">FECHA LÍMITE DE EMISIÓN CADUCADA</span></span></li>';
						}
					}
				}
			}
