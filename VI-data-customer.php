...
if(isset($_GET['idCl']) && isset($_GET['idd'])){
	...
	$sqlUp = 'select 
		...
		svd.referido_por
	from
		s_vi_cot_cabecera as svc
			inner join
		s_vi_cot_detalle as svd ON (svd.id_cotizacion = svc.id_cotizacion)
			inner join
		s_vi_cot_cliente as scl ON (scl.id_cliente = svd.id_cliente)
			inner join 
		s_entidad_financiera as sef ON (sef.id_ef = svc.id_ef)
	where
		svc.id_cotizacion = "' . base64_decode($_GET['idc']) . '"
			and scl.id_cliente = "'.base64_decode($_GET['idCl']).'"
			and svd.id_detalle = "' . base64_decode($_GET['idd']) . '"
...
if($swCl === FALSE){
	$sqlCl = 'select 
		...
		scl.tipo
	from
		s_vi_cot_cliente as scl
			inner join
		s_vi_cot_detalle as sdd ON (sdd.id_cliente = scl.id_cliente)
			inner join
		s_vi_cot_cabecera as sdc ON (sdc.id_cotizacion = sdd.id_cotizacion)
			inner join
...
<?php
				...
				if(($rsSc = $link->query($sqlSc,MYSQLI_STORE_RESULT))){
					if($rsSc->num_rows === 1){
						...
						$dc_weight = $rowSc['cl_peso'];
						$dc_height = $rowSc['cl_estatura'];
		...
		<table class="list-cl">
			<thead>
				<tr>
					...
					<td style="width:16%;">Apellido Paterno</td>
					<td style="width:16%;">Apellido Materno</td>
...
?>
	<!--<div class="content-input" style="<?=$display_type;?> width:30%; margin: 0 auto;">
		<label class="check" style="width:auto;">
	    	<input type="radio" id="dc-type-s" name="dc-type" value="1" <?=$check01;?>>
	    		&nbsp;Es Cliente
	    	</label>
		<label class="check" style="width:auto;">
	    	<input type="radio" id="dc-type-n" name="dc-type" value="0" <?=$check02;?>>
	    		&nbsp;No es Cliente
	    </label><br>
	</div><br>-->
...
?>
	...
	--><div class="form-col">
		...
		<textarea id="dc-address" name="dc-address" class="not-required fbin"><?=$dc_address;?></textarea><br>
...
?>
		...
		<label style="width:auto;">Descripción Ocupación: </label><br>
		<textarea id="dc-desc-occ" name="dc-desc-occ" class="not-required fbin"><?=$dc_desc_occ;?></textarea><br>
...
?>
	...
	<input type="hidden" id="pr" name="pr" value="<?=base64_encode('VI|02');?>">
...
$(document).ready(function(e) {
	...
	$("#dc-next").click(function(e){
		e.preventDefault();
		location.href = 'vi-quote.php?ms=<?=$_GET['ms'];?>&page=<?=$_GET['page'];?>&pr=<?=base64_encode('VI|03');?>&idc=<?=$_GET['idc']?>';
	});
