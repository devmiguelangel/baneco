...
$sap="select sdep.departamento, 'Nueva' as tipo_poliza, sde.cuenta_1, sde.tarjeta, 43 as cod_poliza, 
concat(sc.nombre, ' ', sc.paterno, ' ', sc.materno, ' ', sc.ap_casada) as contratante, 
concat(sc.nombre, ' ', sc.paterno, ' ', sc.materno, ' ', sc.ap_casada) as asegurado, 
'Accidentes Personales' as producto, sca.no_poliza, sc.edad, sc.fecha_nacimiento, sca.fecha_emision, 
sca.fecha_emision as inicio_vigencia, ADDDATE(sca.fecha_emision, INTERVAL 1 YEAR) as termino_vigencia, 
0 as recargo, sp.prima_anual, case sca.forma_pago when 'DA' then 'Debito Automatico' when 'CO' then 'Pago al Contado' when 'DM' then 'Debito Manual' end as forma_pago, 
sus.nombre as vendedor, sag.agencia, sca.no_emision, sp.nombre as plan_nombre, sp.plan, 
case sca.periodo when 'M' then 'Mensual' when 'Y' then 'Anual' end as periodo, 
datediff(curdate(),sac.fecha_cuota) as d_mora, sc.telefono_domicilio, sc.telefono_oficina, sc.telefono_celular, sc.direccion
from 
						...
						s_agencia as sag ON (sag.id_agencia = sus.id_agencia)
							inner join
						s_usuario as su ON (su.id_usuario = sca.and_usuario)
where
						sca.anulado = 0 and 
						sca.emitir = 1 and 
						sac.cobrado = 0 and			
...
$shtml="<table border='0' cellspacing='1' cellpadding='0' style='width:150%; font-size:9pt;'>";
			$shtml.='<tr><td colspan="23" align="center">REPORTE DE MORA MAYOR A 60 DIAS</td></tr>';
			...
			$shtml.='<tr style="background:#D3DCE3;">
					  ...
					  <td align="center"><b>Telefono Domicilio</b></td>
					  <td align="center"><b>Telefono Celular</b></td>
					  <td align="center"><b>Telefono Oficina</b></td>
					  <td align="center"><b>Direccion Domicilio</b></td>
					  ...
					  <td align="center"><b>Dias en Mora</b></td>
					 </tr>';
				while($rap = $cap->fetch_array(MYSQL_ASSOC)){
					$capital = json_decode($rap['plan'], true);
						 ...
						 $shtml.='<tr>
							  ...
							  <td align="center">'.$rap['telefono_domicilio'].'</td>
							  <td align="center">'.$rap['telefono_celular'].'</td>
							  <td align="center">'.$rap['telefono_oficina'].'</td>
				...
				while($rvi = $cvi->fetch_array(MYSQL_ASSOC)){
					$capital = json_decode($rvi['plan'], true);
						 ...
						 $shtml.='<tr>
							  ...
							  <td align="center">'.$rvi['telefono_domicilio'].'</td>
							  <td align="center">'.$rvi['telefono_celular'].'</td>
							  <td align="center">'.$rvi['telefono_oficina'].'</td>
...
$sfile=$scarpeta."/Reporte de Mora.xls"; //ruta del archivo a generar
$fp=fopen($sfile,"w");
