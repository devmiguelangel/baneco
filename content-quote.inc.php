...
if ($seguro === 'VI' || $seguro === 'AP') {
	$cp = true;
	$cp_display = 'display: none;';
	$cp_selected[0] = '';
	$cp_selected[1] = 'checked';
}
?>
...
?>
		<div style="width:70%; margin:0 auto;">
			<div style="<?=$cp_display;?>">
				<input type="radio" id="fq-cp-1" name="fq-cp" value="<?=md5(1);?>" 
					<?=$cp_selected[0];?>>
				<label class="check" style="width:auto; vertical-align:bottom;">
...
?>
        <a href="<?=strtolower($seguro);?>-quote.php?ms=<?=
        	$_GET['ms'];?>&page=<?=$_GET['page'];?>&pr=<?=
        	base64_encode($seguro.'|01');?>" class="quote-ln">Cotizar</a>
    </form>
