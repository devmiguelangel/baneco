...
class CertificateQuery extends CertificateHtml {
	...
	private function get_query_sc(){
		...
		case 'TRM';
		    ...
		    break;
	    case 'AP';
		    $this->set_query_ap_sc();
		    break;
	    case 'VI';
		    $this->set_query_vi_sc();
		    break;
	    }
	}
	...
	private function get_query_ce() {
		...
		case 'TH';
		    ...
		    break;
	   	case 'AP';
			$this->set_query_ap_em();
		    break;
	   	case 'VI';
			$this->set_query_vi_em();
		    break;
		}
	}
	...
	private function set_query_ap_sc(){		//ACCIDENTES PERSONALES
	  $this->sqlPo="select
			...
			scc.fecha_creacion,
            (select
					count(acc.id_cliente)
				from
		            s_ap_cot_detalle as scd
		        inner join
		            s_ap_cot_cliente as acc ON (acc.id_cliente = scd.id_cliente)
				where
					scd.id_cotizacion = scc.id_cotizacion and acc.tipo = 1) as tipo,
			spl.descripcion as desc_plan
        from
            s_ap_cot_cabecera as scc
            	inner join
			s_plan as spl ON (spl.id_plan = scc.id_plan)	
                inner join
            s_usuario as sus ON (sus.id_usuario = scc.id_usuario)
                inner join
            s_ef_compania as efc on (efc.id_ef=scc.id_ef)
        ...
        ";
	  //echo $this->sqlPo;
		if($this->rsPo = $this->cx->query($this->sqlPo, MYSQLI_STORE_RESULT)){	//ACCIDENTES PERSONALES
		  	if($this->rsPo->num_rows === 1){
			 $this->rowPo = $this->rsPo->fetch_array(MYSQLI_ASSOC);
			 $this->rsPo->free();
			 ...
			 $this->sqlDt="select
                ...
                if(sdep.codigo = 'PE',
                    ...
                    concat(acc.ci,
                            ...
                            sdep.codigo)) as ci,
                acc.tipo_documento,
                 case acc.estado_civil
                    when 'SOL' then 'Soltero(a)'
                    when 'CAS' then 'Casado(a)'
                    when 'VIU' then 'Viudo(a)'
                    when 'DIV' then 'Divorciado(a)'
                end as estado_civil,
                sdto.departamento as lugar_residencia,
                acc.localidad,
                acc.direccion,
                ...
                acc.edad,
				scd.fecha_cancelacion as fecha_creacion
            from
                s_ap_cot_detalle as scd
                    inner join
                s_ap_cot_cliente as acc ON (acc.id_cliente = scd.id_cliente)
	...
	private function set_query_ap_em(){		//ACCIDENTES PERSONALES
	  	$this->sqlPo = 'SELECT 
		    ...
		    sdep.departamento AS user_departamento,
		    sae.id_certificado AS id_certificado,
		    sae.fecha_emision,
		    sae.prima,
		    sae.forma_pago,
		    sae.periodo,
			spl.plan,
			spl.nombre as plan_nombre,
			su.nombre as u_nombre,
			su.email as u_email
		FROM
		    s_ap_em_cabecera AS sae
		        LEFT JOIN
		    s_ap_cot_cabecera AS sac ON (sac.id_cotizacion = sae.id_cotizacion)
		        INNER JOIN
			 	...
			 	$this->sqlDt="SELECT 
				    ...
				    IF(sdep.codigo = 'PE',
				        CONCAT(sc.tipo_documento,
				                ...
				                ' ',
				                sc.complemento),
				        CONCAT(sc.ci, ' ', sc.complemento)) AS ci,
				    sdep.codigo AS ext,
				    sc.pais,
				    sc.fecha_nacimiento,
				    sc.lugar_nacimiento,
				    ...
				    sc.edad,
				    sc.email,
				    sc.direccion_laboral AS lugar_trabajo,
				    sad.id_detalle,
				    sc.direccion,
				    sad.cuenta_1,
				    sad.cuenta_2,
				    ...
				    sar.observacion
				FROM
					s_ap_em_detalle AS sad
						INNER JOIN
				    s_cliente AS sc ON (sc.id_cliente = sad.id_cliente)
				    	INNER JOIN
				    s_ap_em_respuesta sar ON (sar.id_detalle = sad.id_detalle) 
				        INNER JOIN
				    s_departamento AS sdep ON (sdep.id_depto = sc.extension)
				        LEFT JOIN
				    s_departamento AS sdto ON (sdto.id_depto = sc.lugar_residencia)
                	...
                	if ($this->rsDt->num_rows > 0) {
						...
						FROM
						    s_ap_beneficiario AS sab
						        INNER JOIN
						    s_ap_em_detalle AS sad ON (sad.id_detalle = sab.id_detalle)
						WHERE
						    sad.id_emision = "' . $this->rowPo['id_emision'] . '"
						;';	
						// echo $this->sqlBs;
	...
	private function set_query_vi_sc(){		//VIDA INDIVIDUAL
		$this->sqlPo="select
            ...
            (select
					count(acc.id_cliente)
				from
		            s_vi_cot_detalle as scd
		        inner join
		            s_vi_cot_cliente as acc ON (acc.id_cliente = scd.id_cliente)
				where
					scd.id_cotizacion = scc.id_cotizacion and acc.tipo = 1) as tipo,
			spl.descripcion as desc_plan
        from
            s_vi_cot_cabecera as scc
            	inner join
			s_plan as spl ON (spl.id_plan = scc.id_plan)
                inner join
            s_usuario as sus ON (sus.id_usuario = scc.id_usuario)
        ...
        ";
	  // echo $this->sqlPo;
		if($this->rsPo = $this->cx->query($this->sqlPo, MYSQLI_STORE_RESULT)){	//BANCA SEGURO
		  	if($this->rsPo->num_rows === 1){
			 $this->rowPo = $this->rsPo->fetch_array(MYSQLI_ASSOC);
			 $this->rsPo->free();
	...
	private function set_query_vi_em(){		//VIDA INDIVIDUAL
		...
		FROM
		    s_vi_em_cabecera AS sve
		    	LEFT JOIN
		    s_vi_cot_cabecera AS svc ON (svc.id_cotizacion = sve.id_cotizacion)
		        INNER JOIN
		    s_entidad_financiera AS sef ON (sef.id_ef = sve.id_ef)
		        INNER JOIN
		    s_compania AS scia ON (scia.id_compania = sve.id_compania)
		        INNER JOIN
		    s_poliza AS sp ON (sp.id_poliza = sve.id_poliza)
		        INNER JOIN
		...
		if ($this->rsPo = $this->cx->query($this->sqlPo, MYSQLI_STORE_RESULT)) {
			if ($this->rsPo->num_rows === 1) {
				...
				FROM
				    s_cliente AS sc
				        INNER JOIN
				    s_vi_em_detalle AS svd ON (svd.id_cliente = sc.id_cliente)
				    	INNER JOIN
				    s_vi_em_respuesta AS svr ON (svr.id_detalle = svd.id_detalle)
				        INNER JOIN
				    s_departamento AS sdep ON (sdep.id_depto = sc.extension)
				        LEFT JOIN
				    s_departamento AS sdto ON (sdto.id_depto = sc.lugar_residencia)
				    ...
				    s_ocupacion AS sdo ON (sdo.id_ocupacion = sc.id_ocupacion)
				WHERE
				    svd.id_emision = '" . $this->rowPo['id_emision'] . "';";
			    	...
			    	if($this->rsDt->num_rows > 0){
						...
						FROM
						    s_vi_beneficiario AS sab
						        INNER JOIN
						    s_vi_em_detalle AS sad ON (sad.id_detalle = sab.id_detalle)
						WHERE
						    sad.id_emision = "' . $this->rowPo['id_emision'] . '"
						;';	
						// echo $this->sqlBs;
