...
class SibasDB extends MySQLi
{
    ...
    public
        ...
        $payment = array(
            0 => 'CO|Pago al Contado',
            1 => 'DA|Débito Automatico'),
        $period = array(
            0 => 'Y|Pago Anual',
            1 => 'M|Pago Mensual'),
        $state_account = array(
        ...
        $prima = array(
            'AP' => array(
                'Plan A' => array(
                    'PN' => 84.88,      //Prima neta
                    'IM' => 20.15,      //Impuesto
                    'CC' => 2.97,       //Comision Corretaje
                    'CS' => 33.58       //Comision Sponsor
                ),
                'Plan B' => array(
                    'PN' => 93.76,
                    'IM' => 22.43,
    ...
    public function verify_agency($kwargs)
    {
        $this->sql = 'SELECT 
            sa.id_agencia,
            sa.codigo AS ag_codigo,
            sa.agencia AS ag_nombre,
            sa.id_depto AS ag_depto
        FROM
            s_agencia AS sa
                INNER JOIN
            s_entidad_financiera AS sef ON (sef.id_ef = sa.id_ef)
                LEFT JOIN
            s_departamento AS sdep ON (sdep.id_depto = sa.id_depto)
        WHERE
            sa.id_agencia LIKE "%' . $kwargs['ag_id'] . '%"
                AND sa.codigo LIKE "%' . $kwargs['ag_code'] . '%"
        ;';
    ...
    public function verifyCollections($pr, $ide)
    {
        $this->sql = "SELECT 
            sc.id_cobranza
        FROM
            s_" . $pr . "_cobranza AS sc
        WHERE
            sc.id_emision = '" . $ide . "'
        ;";
    ...
    public function notIssue($pr, $ide)
    {
        $this->sql = "UPDATE s_" . $pr . "_em_cabecera AS sem 
        SET 
            sem.emitir = FALSE
        WHERE
            sem.id_emision = '" . $ide . "'
    ...
    public function getAmountPledge($period, $prima)
    {
        $amount = $prima;
    ...
    public function check_policy_pp($user, $pr, $policy)
    {
        $this->sql = 'select 
            srp.id_rango
        from
            ...
            s_agencia as sag ON (sag.id_agencia = su.id_agencia)
                inner join
            s_rango_poliza as srp ON (srp.id_agencia = su.id_agencia and 
                    srp.id_depto = su.id_depto)
        where
            su.id_usuario = "' . base64_decode($user) . '"
                and srp.producto = "' . $pr . '"
                and ' . $policy . ' between srp.poliza_inicial and srp.poliza_final
        ;';
    ...
    public function verify_customer($dni, $ext, $idef, $pr = 'DE')
    {
        ...
        case 'TH':
            ...
            break;
        case 'AP':
            $table = 's_ap_cot_cliente as sc';
            break;
        case 'VI':
            $table = 's_vi_cot_cliente as sc';
            break;
        }
    ...
    public function number_clients_AP($idc, $idef, $flag)
    {
        $this->sql = 'select 
            sdc.id_cotizacion, COUNT(scl.id_cliente) as numCl
        from
    ...
    public function number_clients_VI($idc, $idef, $flag)
    {
        $this->sql = 'select 
            sdc.id_cotizacion, COUNT(scl.id_cliente) as numCl
        from
    ...
    public function get_question_be($ide, $pr)
    {
        $this->sql = 'select 
            sar.id_respuesta,
            sar.id_detalle,
        ...
        from
            s_' . strtolower($pr) . '_em_cabecera as sae
                inner join
            s_' . strtolower($pr) . '_em_detalle as sad ON (sad.id_emision = sae.id_emision)
                inner join
            s_' . strtolower($pr) . '_em_respuesta as sar ON (sar.id_detalle = sad.id_detalle)
        where
            sae.id_emision = "' . base64_decode($ide) . '"
        limit 0, 1
        ;';
    ...
    public function get_plan($idef, $product, $id = '')
    {
        $token = true;
        ...
        from 
            s_plan as sp
                inner join 
            s_entidad_financiera as sef ON (sef.id_ef = sp.id_ef)
        where sp.producto = "' . $product . '"
            and sef.id_ef = "' . base64_decode($idef) . '"
            and sef.activado = true
            and sp.id_plan like "' . $id . '"
    ...
    public function client_plan($pr, $ci, $ext, $plan)
    {
        $plan_em = array();
        $plan_co = array();
        ...
        if ($plan !== false) {
            $plan_co = json_decode($plan['plan'], true);

            foreach ($plan_co as $key => $value) {
                if ((int)$value['rank'] > $rank_co) {
                    $rank_co = (int)$value['rank'];
                }
            }

            $this->sql = 'SELECT 
                sae.id_emision, sp.plan
            FROM
                s_' . strtolower($pr) . '_em_cabecera AS sae
                    INNER JOIN
                s_' . strtolower($pr) . '_em_detalle AS sad ON (sad.id_emision = sae.id_emision)
                    INNER JOIN
                s_cliente AS sc ON (sc.id_cliente = sad.id_cliente)
                    INNER JOIN
            ...
            if (($this->rs = $this->query($this->sql, MYSQLI_STORE_RESULT)) !== false) {
                if ($this->rs->num_rows > 0) {
                    while ($this->row = $this->rs->fetch_array(MYSQLI_ASSOC)) {
                        $plan_em = json_decode($this->row['plan'], true);

                        $rank = 0;
                        foreach ($plan_em as $key => $value) {
                            if ((int)$value['rank'] > $rank) {
                                $rank = (int)$value['rank'];
                            }
                        }
        ...
        switch($row['estado']){
            case 'A':
                ...
                if($token < 2){
                    ...
                    && (boolean)$row['emitir'] === true
                    && (boolean)$row['anulado'] === false) {
                    $arr_state['action'] = 'Editar';
                    $arr_state['link'] = $pr . '-quote.php?ms=' . md5('MS_' . $product) 
                        ...
                        . '&target='.md5('edit');
                    if ((boolean)$row['pp'] === true) {
                        $arr_state['link'] .= '&print=' . sha1('p_print');
                    }
                }
    ...
    public function getRegistrationNumber ($idef, $product, $token, $prefix = '') 
    {
        $arrTable = array (
            ...
            'AP'    => array (0 => 's_ap_cot_cabecera',     1 => 's_ap_em_cabecera',    2 => ''),
            'VI'    => array (0 => 's_vi_cot_cabecera',     1 => 's_vi_em_cabecera',    2 => ''),
            );
    ...
    public function check_no_policy($pr, $no_poliza)
    {
        $this->sql = 'select 
            sae.id_emision 
        from
    ...
    public function getFinancialInstitutionCompany($idef, $pr = '')
    {
        $this->sql = 'select 
