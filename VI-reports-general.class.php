...
class ReportsGeneralVI{
	...
	private function get_query_report(){
		...
		case md5('RG'):
			$this->token = 'RG';
			$this->xlsTitle = 'Vida Individual - Reporte General'; break;
		case md5('RP'):
			$this->token = 'RP';
			$this->xlsTitle = 'Vida Individual - Reporte Polizas Emitidas'; break;
		case md5('RQ'):
			$this->token = 'RQ';
			$this->xlsTitle = 'Vida Individual - Reporte Cotizaciones'; break;
		case md5('IQ'):
			$this->token = 'IQ';
			$this->xlsTitle = 'Vida Individual - Cotizaciones'; break;
		case md5('PA'):
			$this->token = 'PA';
			$this->xlsTitle = 'Vida Individual - Reporte Polizas Preaprobadas'; break;
		case md5('AN'):
			$this->token = 'AN';
			$this->xlsTitle = 'Vida Individual - Reporte Polizas Emitidas'; break;
		case md5('IM'):
			$this->token = 'IM';
			$this->xlsTitle = 'Vida Individual - Preaprobadas'; break;
		case md5('UF'):
			$this->token = 'UF';
			// $this->xlsTitle = 'Accidentes Personales - Reporte Polizas Emitidas'; 
			break;
		case md5('RC'):
			$this->token = 'RC';
			$this->xlsTitle = 'Vida Individual - Reporte Cobranzas'; break;
		case md5('RE'):
			$this->token = 'RE';
	...
	private function set_query_de_report(){
		...
		if($this->token === 'RG'){
			...
			";
			if (!empty($this->data['r-mora']) && $this->token === 'RC') {
				$this->sql .= "and datediff(curdate(), svc.fecha_cuota) 
	...
	private function set_query_de_report_quote(){
		...
		from
			...
			s_vi_cot_detalle as sdd ON (sdd.id_cotizacion = sdc.id_cotizacion)
				inner join
			s_vi_cot_cliente as sc ON (sc.id_cliente = sdd.id_cliente)
				inner join
...
<?php
		...
		while($this->row = $this->rs->fetch_array(MYSQLI_ASSOC)){
			...
			from
				s_cliente as sdc
					inner join
				s_vi_em_detalle as sdd ON (sdd.id_cliente = sdc.id_cliente)
					inner join
...
<?php if (empty($this->row['po_archivo']) === true && $this->row['anulado'] == 0): ?>
				<form action="upload-file.php" method="post" enctype="multipart/form-data" 
					...
					<input type="hidden" name="product" value="<?=base64_encode('VI');?>">
...
<?php
		...
		while($this->row = $this->rs->fetch_array(MYSQLI_ASSOC)){
			...
			from
				s_vi_cot_cliente as sdc
					inner join
				s_vi_cot_detalle as sdd ON (sdd.id_cliente = sdc.id_cliente)
					inner join
				s_departamento as sdep ON (sdep.id_depto = sdc.extension)
			where
				sdc.id_ef = '".$this->data['idef']."'
...
?>
		<tr style=" <?=$bg;?> " class="row quote" rel="0"
            ...
            <td><?=$this->row['r_sucursal'];?></td>
            <td><?=htmlentities($this->row['r_agencia'], ENT_QUOTES, 'UTF-8');?></td>
            <!--<td><a href="detalle-cotizacion/detalle-certificado.php?idcotiza=<?=base64_encode($this->row['idc']);?>&cat=<?=base64_encode('VI');?>&type=PRINT" class="fancybox fancybox.ajax observation">Ver Slip de Cotización</a></td>-->
        </tr>
<?php
					}
				}
