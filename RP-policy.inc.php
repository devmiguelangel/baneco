...
<?php if ($pr === 1): ?>
	$('.rp-link[rel=1]').trigger('click');
	$('.f-reports #pr[value="AP"]').trigger('submit');
<?php elseif ($pr === 2): ?>
	$('.rp-link[rel=2]').trigger('click');
	$('.f-reports #pr[value="VI"]').trigger('submit');
<?php endif ?>
});
</script>
...
?>
			...
			<label class="lbl-cb"><input type="radio" id="frp-canceled-no" name="frp-canceled-p" value="0" checked>&nbsp;NO</label>
            <label class="lbl-cb"><input type="radio" id="frp-canceled-both" name="frp-canceled-p" value="">&nbsp;Todos</label>
